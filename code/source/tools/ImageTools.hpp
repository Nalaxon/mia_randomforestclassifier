...
namespace ImageTools
{
  ...
  static void extractSamples(const std::vector<cv::Mat> &labelImgs, const std::vector<cv::Mat> &trainImgs, std::vector<Sample<Label, cv::Mat>> &samples, 
    const cv::Size &patch_size, int xdiff = -1, int ydiff = -1, int borderType = -1, const cv::Scalar &value = cv::Scalar())
  {
