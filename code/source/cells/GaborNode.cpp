...
GaborNode::Direction GaborNode::split(const std::vector<cv::Mat>& mat, const cv::Rect& roi) const
{
	int kernel_size = mat[0].cols / 3;
	cv::Mat kernel = cv::getGaborKernel(cv::Size(kernel_size, kernel_size), m_sig, m_th, m_lm, m_gm, m_ps);

	cv::Mat dest;
	cv::filter2D(mat[0], dest, CV_32F, kernel);
